...
void Engine::Camera::Update(float dt)
{
    ...
    case(ViewMode::VIEW_PERSPECTIVE):
        ...
        ComputeViewProj(position, direction);
        break;
...
{
    XMStoreFloat3(&m_Position, XMVectorSet(0, 0, -4, 1));
    XMStoreFloat3(&m_TargetPos, XMVectorSet(0, 0, 0, 1));
...
void Engine::Camera::ComputeViewProj(FXMVECTOR position, FXMVECTOR direction)
{
    XMVECTOR upVector = XMLoadFloat3(&m_UpVector);

    //Work out the resultant view matrix
    XMMATRIX viewMatrix = XMMatrixLookAtLH(position, direction, upVector);
...
void Engine::Camera::SetDirection(float x, float y, float z)
{
    m_Direction.x = x;
    m_Direction.y = y;
    m_Direction.z = z;
...
void Engine::Camera::SetDirection(DirectX::FXMVECTOR direction)
{
    XMStoreFloat3(&m_Direction, direction);
}

DirectX::XMFLOAT3& Engine::Camera::Direction()
{
    return (m_Direction);
}
...
void Engine::Camera::SetOrbitalAxis(float radius, float theta, float phi)
{
    ...
    //m_Phi = XMConvertToRadians(phi);

    //
    ////Clamp Theta (-180 < theta < 180)
    //if (m_Theta < XMConvertToRadians(180)) {
    //    m_Theta = XMConvertToRadians(-180);
    //}
    //if (m_Theta > XMConvertToRadians(-180)) {
    //    m_Theta = XMConvertToRadians(180);
    //}
    ...
    ////Clamp Phi (-90 < phi < 90)
    //if (m_Phi < XMConvertToRadians(90)) {
    //    m_Phi = XMConvertToRadians(-90);
    //}
    //if (m_Phi > XMConvertToRadians(-90)) {
    //    m_Phi = XMConvertToRadians(90);
    //}
...
void Engine::Camera::Orbit()
{
    ...
    //Ensure the radius isn't less than or equal to 0
    if (m_Radius <= 0) {
        m_Radius = 0.1f;    
    }
